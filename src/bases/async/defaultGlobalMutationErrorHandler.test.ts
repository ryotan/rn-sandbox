...
describe('defaultGlobalMutationErrorHandler', () => {
  const axiosError = new AxiosError('error', '', undefined, undefined, {
    status: 500,
  ...
  test('500 Internal Server Errorの場合に予期せぬエラーのスナックバーを表示', () => {
    ...
    const mutation = {} as unknown as Mutation<unknown, unknown, unknown>;
    const onUnauthorized = jest.fn();
    const errorHandler = defaultGlobalMutationErrorHandler(new QueryClient(), onUnauthorized);
  ...
  test('disableGlobalErrorHandlerが設定されている場合に何も行わない', () => {
    const mutation = {meta: {disableGlobalErrorHandler: true}} as unknown as Mutation<unknown, unknown, unknown>;
    const spySnackbar = jest.spyOn(Snackbar, 'show').mockImplementation(() => {});
    const onUnauthorized = jest.fn();
    const errorHandler = defaultGlobalMutationErrorHandler(new QueryClient(), onUnauthorized);
    expect(errorHandler).not.toBeUndefined();
